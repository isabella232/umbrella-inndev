...
function clean_contact_send($atts) {
	...
	$options = get_option('clean_contact_router');
	if ( $options ) {
	...
	if ( isset( $_POST['clean_contact_router'] ) && array_key_exists( stripslashes($_POST['clean_contact_router']), $subject_options) ) {
		$to_email = clean_contact_scrub( $subject_options[ stripslashes($_POST['clean_contact_router']) ] );
	}
...
function clean_contact_strings() {
	...
	if ( $options ) {
		...
		$strings['str_clean_contact_router'] = esc_attr( implode('|', $subject_options) );
	}
